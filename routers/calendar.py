...
def upcoming_events(request: Request, service=Depends(get_calendar_service)):
    events = service.events().list(calendarId='primary', maxResults=10, orderBy='startTime', singleEvents=True).execute()
...
def today_events(request: Request, service=Depends(get_calendar_service)):
    ...
    events = service.events().list(calendarId='primary', timeMin=now, timeMax=later, singleEvents=True).execute()
